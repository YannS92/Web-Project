...
if($_SERVER['REQUEST_METHOD'] == 'POST' || 'OPTIONS'){
    ...
    if(!empty($donnees->centre_pilote) && !empty($donnees->promotions_pilote) && !empty($donnees->NOM) && !empty($donnees->PRENOM) && !empty($donnees->Login) && !empty($donnees->Mot_de_passe) && !empty($donnees->ID_Utilisateur_Administrateur)){
